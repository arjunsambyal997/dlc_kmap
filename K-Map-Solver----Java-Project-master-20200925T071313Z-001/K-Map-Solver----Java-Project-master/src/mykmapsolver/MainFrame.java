...
public class MainFrame extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jPanel2.setBounds(450, 500, 450, 270);
        ...
        nVar.setMaximumRowCount(4);
        nVar.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "2", "3", "4" }));
...
// TODO add your handling code here:
	...
	public void paint (java.awt.Graphics g)
	{
		...
		int x_coord[]=new int[row];
		int count_x=0;
		
		int init_val[]=new int[col];
		for(int i = 0; i < row; i++)
		{
			for(int j = 0; j < col; j++)
			{
		 ...
		 *
		 */
		X = originX+(length-(2+col)*cellSize)/2;
		Y = originY+(height-(2+row)*cellSize)/2;
			...
			/**	if(c=='0')
				{
					...
					if(j+1!=col && count_x==x_coord[j+1] && count_x>0 ) {
					
					g2.drawRect (X+cellSize*(j+2)+5, Y+cellSize*(init_val[j]+2), (cellSize*count_x), (cellSize*count_x));
					
					System.out.println("consecutive paae hen "+ count_x);
					}else if(count_x>0) {
						g2.drawRect (X+cellSize*(j+2)+5, Y+cellSize*(init_val[j]+2), (cellSize*count_x+cellSize*count_x), 10);
						g2.setColor(new Color(200, 200, 0));
						System.out.println("gya"+j);
					}
					x_coord[j]=0;
					init_val[j]=i;
				...
				if(c=='1') {
					...
					g2.drawRect (X+cellSize*(j+2)+3, Y+cellSize*(i+3)-16, cellSize, 15);
				}
                                if(c=='x') {
				...
				// g2.drawRect (X+cellSize*(1+2)+5, Y+cellSize*(0+2), (cellSize*3), 10);
